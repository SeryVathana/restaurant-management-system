...
export const login = createAsyncThunk("auth/login", async (payload: any) => {
  const response = await axios.post("https://restaurant-management-system-e4qi.onrender.com/auth/customer/login", payload);
...
export const register = createAsyncThunk("auth/register", async (payload: any) => {
  console.log(payload);
  const response = await axios.post("https://restaurant-management-system-e4qi.onrender.com/auth/customer/register", {
    email: payload.email,
